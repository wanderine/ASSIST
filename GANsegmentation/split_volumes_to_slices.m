...
for subject = 1:length(subjects)
    if not(contains(subjects(subject).name,'BraTS20'))
...
% Do a random permutation of subjects, to get training data from all 19 sites
permutation = randperm(length(subjects));
subjects = subjects(permutation);

save('subjectPermutation.mat','permutation','subjects')

% Need to do all splits in one go, because otherwise randperm will give different divisions for different settings...
% We are saving the exact divisions in the filenames, as we use the original subject numbers

for augmentation = 1:2
    if augmentation == 1
    ...
    for channels = 1:2
        ...
        for numberOfSubjects = [369 313 184 92 46 23]
            
            numberOfSubjects

            slicesPerSubject = zeros(numberOfSubjects,1);
            
            if useAugmentation && not(separateSegmentationChannels)
                dataset = ['brats2020_' num2str(numberOfSubjects) 'subjects_rotationaugmentation_singlesegmentationchannel_min15percentcoverage'];
            elseif not(useAugmentation) && not(separateSegmentationChannels)
                dataset = ['brats2020_' num2str(numberOfSubjects) 'subjects_noaugmentation_singlesegmentationchannel_min15percentcoverage'];
            elseif useAugmentation && separateSegmentationChannels
                dataset = ['brats2020_' num2str(numberOfSubjects) 'subjects_rotationaugmentation_multiplesegmentationchannels_min15percentcoverage'];
            elseif not(useAugmentation) && separateSegmentationChannels
                dataset = ['brats2020_' num2str(numberOfSubjects) 'subjects_noaugmentation_multiplesegmentationchannels_min15percentcoverage'];
            end
            ...
            for subject = 1:numberOfSubjects
                ...
                for z = 1:sz
                    ...
                    if highpixels/(256*256) > 0.15
                        ...
                        imwrite(uint8(slice_T1),[basepath_png 'T1/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_T1.png'],'png');
                        imwrite(uint8(slice_T2),[basepath_png 'T2/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_T2.png'],'png');
                        imwrite(uint8(slice_T1ce),[basepath_png 'T1CE/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_T1ce.png'],'png');
                        imwrite(uint8(slice_flair),[basepath_png 'FLAIR/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_flair.png'],'png');
                        
                        if not(separateSegmentationChannels)
                            imwrite(uint8(slice_seg),[basepath_png 'Seg/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_seg.png'],'png');
                        else
                            imwrite(uint8(slice_seg_onehot(:,:,1)),[basepath_png 'Seg1/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_seg_channel1.png'],'png');
                            imwrite(uint8(slice_seg_onehot(:,:,2)),[basepath_png 'Seg2/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_seg_channel2.png'],'png');
                            imwrite(uint8(slice_seg_onehot(:,:,3)),[basepath_png 'Seg3/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_seg_channel3.png'],'png');
                        end
                        ...
                        if useAugmentation
                            ...
                            for augmentation = 1:9
                                ...
                                imwrite(uint8(rotated_T1),[basepath_png 'T1/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_T1.png'],'png');
                                imwrite(uint8(rotated_T2),[basepath_png 'T2/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_T2.png'],'png');
                                imwrite(uint8(rotated_T1ce),[basepath_png 'T1CE/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_T1ce.png'],'png');
                                imwrite(uint8(rotated_flair),[basepath_png 'FLAIR/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_flair.png'],'png');
                                
                                if not(separateSegmentationChannels)
                                    rotated_seg = imrotate(slice_seg,randomRotation,'nearest','crop'); % nearest for annotations
                                    imwrite(uint8(rotated_seg),[basepath_png 'Seg/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_seg.png'],'png');
                                else
                                    ...
                                    imwrite(uint8(rotated_seg1),[basepath_png 'Seg1/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_seg_channel1.png'],'png');
                                    imwrite(uint8(rotated_seg2),[basepath_png 'Seg2/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_seg_channel2.png'],'png');
                                    imwrite(uint8(rotated_seg3),[basepath_png 'Seg3/' 'Subject_' subjects(subject).name(end-2:end) '_slice_' num2str(z) '_augmentation_' num2str(augmentation) '_seg_channel3.png'],'png');
